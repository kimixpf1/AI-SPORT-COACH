        ...
        <header className="rounded-3xl border border-white/10 bg-gradient-to-r from-slate-900 via-slate-900 to-indigo-950 p-6 shadow-2xl shadow-indigo-950/30">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-3">
              ...
              <div>
                <h1 className="text-3xl font-semibold tracking-tight sm:text-4xl">AI 运动教练</h1>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {['上传自拍视频', '本地识别骨架', '输出数据图表', '生成纠错建议'].map((item) => (
          ...
          <section className="space-y-6">
            <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-5 shadow-xl shadow-black/20">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-400">推荐上传侧前方拍摄、全身和器械完整入镜、单次动作 5-20 秒的视频。</p>
                </div>
                <button
              ...
              <div className="mt-5 grid gap-4 lg:grid-cols-[1fr_220px]">
                <label className="rounded-2xl border border-dashed border-cyan-400/30 bg-cyan-400/5 p-4">
                  ...
                  {file && (
                    <p className="mt-3 text-sm text-slate-200">已选择 {file.name} · {(file.size / 1024 / 1024).toFixed(1)} MB</p>
                  )}
                </label>
                ...
                <label className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  ...
                  <p className="mt-3 text-xs leading-5 text-slate-400">已知动作时优先手动选择，识别会更稳；不确定时再用自动识别。</p>
                </label>
              </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-5 shadow-xl shadow-black/20">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-400">绿色轨迹显示手部中点路径，适合作为杠铃移动趋势的近似参考。</p>
                </div>
                {trackingData && (
            ...
            {showHistory && (
              <div className="rounded-3xl border border-white/10 bg-slate-900/80 p-5 shadow-xl shadow-black/20">
                ...
                <p className="mt-2 text-sm text-slate-400">Pages 版本使用浏览器本地存储保存最近记录，方便手机和电脑快速回看。</p>
                  ...
                  {latestHistory.length === 0 ? (
                    ...
                    latestHistory.map((item) => (
                      <div key={item.id} className="rounded-2xl border border-white/10 bg-slate-950/70 p-4">
                        ...
                        {item.suggestions[0] && <p className="mt-3 text-sm text-slate-300">下次优先改进：{item.suggestions[0]}</p>}
                      </div>
                    ))
                  )}
                </div>
              ...
              {result ? (
                ...
                <div className="flex min-h-[420px] items-center justify-center rounded-2xl border border-dashed border-white/10 bg-slate-950/70 px-6 text-center">
                  <div>
                    ...
                    <p className="mt-3 text-sm leading-6 text-slate-400">当前版本优先强化手机端微信浏览器与桌面端通用体验，后续还可以继续补充多角度对比、训练周期追踪和更多专项动作。</p>
                  </div>
                </div>
              )}
            </div>
